...
@export var damage_particle: ParticleData
...
# VIDA
# ==============================
func take_damage(amount:int, hit_direction:=Vector2.ZERO):
	...
	spawn_damage_particles(hit_direction)
...
func spawn_footstep_particles(dir: Vector2):
	...
	footstep_timer = footstep_particle.spawn_interval

	# Escolhe o offset dependendo da direção
	var offset := Vector2.ZERO

	if abs(dir.x) > abs(dir.y):

		if dir.x > 0:
			offset = footstep_particle.offset_right
		else:
			offset = footstep_particle.offset_left

	else:

		if dir.y > 0:
			offset = footstep_particle.offset_down
		else:
			offset = footstep_particle.offset_up

	# Cria a quantidade de partículas definida no Resource
	for i in range(footstep_particle.amount):

		var particle = preload("res://ParticleManager/Particle.tscn").instantiate()

		get_tree().current_scene.add_child(particle)

		particle.iniciar(footstep_particle, -dir)
...
func spawn_heal_particles():
	...
	for i in range(heal_particle.amount):
		...
		# direção para cima
		particle.iniciar(heal_particle, Vector2.UP)
...
func spawn_damage_particles(hit_direction: Vector2):
	...
	if hit_direction == Vector2.ZERO:
		...
		base_dir = -hit_direction.normalized()

	for i in range(damage_particle.amount):

		var particle = preload("res://ParticleManager/Particle.tscn").instantiate()
		get_tree().current_scene.add_child(particle)

		var dir = base_dir.rotated(
			deg_to_rad(randf_range(-25.0, 25.0))
		)
